  ...
  */
  async function getPatientIDs(name) {
    ...
    var result = await requestor(
      "POST",
      raw,
      "http://localhost:5000/api/Receptionist/get-patientid-list"
    );
  ...
  PID.addEventListener('change',async()=>{
    const filteredPatients = patientsList.filter(patient => patient.PatientID ===PID.value );
    name.value = filteredPatients[0].Basic['Fname'] +" "+filteredPatients[0].Basic['Mname'] +" "+filteredPatients[0].Basic['Fname']
  })
