...
const updateTimeSlots = asyncHandler(async (req, res, next) => {
  try {
    const { TimeSlotID,  newStartTime, newEndTime ,Hospital} = req.body;
    if(!Hospital){
...
const appointmentStatus = asyncHandler(async (req, res, next)=>{
  try {
    const { Reason,Status,ID, requestedId } = req.body;
...
const updateAppointmentDoctor = asyncHandler(async (req, res, next)=>{
  try {
    const { ID,doctorID ,requestedId } = req.body;
    if (!ID && !Status && !requestedId) {
      throw new Error("ID,Status, requestedId not found");
    }
    console.log(ID);
...
module.exports = { updatePatientData, updateTimeSlots,appointmentStatus,updateAppointmentDoctor };
