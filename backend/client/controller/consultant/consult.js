...
const reportPage = asyncHandler(async(req,res)=>{
  ...
  const result = await requestor(
    "POST",
    raw,
    "http://localhost:5000/api/Receptionist/get-appointment-detail"
...
res.render("consultant/report",{
  title: " ",
    ...
    respiratoryRate:result.VitalsData?result.VitalsData['respiratoryRate']:"--",
    bloodPressure:result.VitalsData?result.VitalsData['bloodPressure']:{systolic:"--"},
    date:date,
    time:time,
    docName:Name,
    ...
    Suggestions:PrescriptionID?PrescriptionID.prescriptions:"No suggestions!",
})
})


module.exports = {
  consultHome,AppointmentTable,PatientDetails,addPrescription,examinDate,reportPage
};
